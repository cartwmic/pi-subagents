...
Foreground and async runners share bounded child-protocol handling. A child JSONL line above 4 MiB fails with structured `protocolError` code `protocol_output_limit`, stderr retains only its latest 128 KiB, split UTF-8 and final unterminated JSON events remain valid, and `agent_end.willRetry` defers completion until the child settles. `agent_settled`, terminal assistant messages, and `compaction_end` are not proof that extension work is finished: completion callbacks and continuation input/preflight hooks can still be pending before another `agent_start`. In JSON-mode children, the required child runtime sends versioned `subagent_host_lifecycle` records (`ready` / `shutdown`) on stderr. Both supervisors then start the unchanged one-second final drain only when the print host reaches `session_shutdown` with reason `quit`, not at low-level settlement. This preserves final-turn/no-continuation cleanup and the existing three-second hard kill and post-exit stdio guards. Older runtimes that cannot advertise the handshake retain the legacy settlement/compaction fallback. Active extension work, like active provider work, remains subject to explicit abort/stop and run timeouts; use `timeoutMs` to bound it. The Pi print host must itself drain extension compaction and continuation promises before teardown (tested with patched Pi 0.85.1); supervision does not replace that host responsibility.
