...
Foreground and async runners share bounded child-protocol handling. A child JSONL line above 4 MiB fails with structured `protocolError` code `protocol_output_limit`, stderr retains only its latest 128 KiB, split UTF-8 and final unterminated JSON events remain valid, and `agent_end.willRetry` defers completion until the child settles. `agent_settled` and terminal assistant messages are cleanup candidates, not proof that extension work is finished: `compaction_start` suspends final drain, `compaction_end` restores pending cleanup, and a new agent/turn cancels it for the continuation. Final-turn compaction still gets bounded lingering-process cleanup. Active compaction, like active provider work, remains subject to explicit abort/stop and run timeouts; the one-second final-output grace is not a compaction deadline.
